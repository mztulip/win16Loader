...
static void init_int3f(unsigned short handler_off)
{
    ...
    kprintf("IDT: phys=0x%05lX int3f=0x%04X int21=0x%04X\n",
            g_idt_phys, handler_off, h21_off);
}
...
{
    serial_init();
    kprintf("=== NE Loader STEP12 - GlobalAlloc + KCB + SKI.EXE ===\n");
    ...
    /* 5. Alokuj fake PSP (256 B), KCB i globalny heap (256KB) */
    {
        unsigned      kcb_seg, heap_seg, psp_seg;
        ...
        kprintf("GlobalHeap: seg=0x%04X phys=0x%05lX size=%luKB\n",
                heap_seg, heap_phys, GLOBAL_HEAP_SIZE / 1024UL);

        kcb = (KCB_LAYOUT __far *)MK_FP(kcb_seg, 0);
        kcb->app_hinstance  = SEL_APP_DATA;      /* 0x40 */
        kcb->next_dyn_sel   = GDYN_FIRST_SEL;    /* 0x128 */

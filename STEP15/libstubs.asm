...
; __U4D: Watcom 32-bit unsigned divide runtime helper
; Input:  DX:AX = dividend,  CX:BX = divisor
; Output: DX:AX = quotient,  CX:BX = remainder
; Note: Only handles cases where divisor fits in 16 bits (CX==0),
;       which covers our usage: dividend <= 66727, divisor = 1193.
global __U4D
__U4D:
    ; Fast path: divisor fits in 16 bits (CX==0)
    test  cx, cx
    jnz   .slow
    ; dividend DX:AX divided by 16-bit BX
    ; Use DIV: DX:AX / BX -> quotient AX, remainder DX
    div   bx            ; AX = quotient lo, DX = remainder
    ; quotient hi = 0, remainder hi = 0
    xor   cx, cx        ; CX:BX = remainder  (BX = DX after next line)
    mov   bx, dx        ; BX = remainder lo
    xor   dx, dx        ; DX = quotient hi
    ret
.slow:
    ; Full 32-bit / 32-bit: not needed for our use case, just return 0
    xor   ax, ax
    xor   dx, dx
    xor   bx, bx
    xor   cx, cx

...
str_title   db "STEP14: GlobalHeap XMS + RAM detection", 0
...
%endif
    ...
    ; Trace: print 'K'/'U'/'G' + func_off(hex) + SP(hex) + '\n' for all DLL calls
    push dx
    push ax
    push bx
    push cx
    ; compute trace char from AX (code_sel: 0x48=K, 0x58=U, 0x68=G)
    mov  bl, al           ; BL = low byte of code_sel
    shr  bl, 4            ; BL = 4/5/6
    sub  bl, 4            ; BL = 0/1/2
    mov  bh, 0
    mov  al, byte [cs:dbg_dll_letters + bx]
...
.tr_c: in   al, dx
    ...
    mov  al, byte [cs:dbg_dll_letters + bx]
    out  dx, al
    ; print SI (func_off) as 4 hex
    mov  ax, si
    call gpf_print_hex4
    ; print SP (current SP value) as 4 hex: BX holds index, save/restore
    push bx
    ; current SP = sp reg; add offset for pushes done in trace so far
    ; just print SS:SP which reflects current depth
    mov  ax, sp
    call gpf_print_hex4
    pop  bx
    ; print '\n'

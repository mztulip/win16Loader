/*
 * loader.c - STEP11: prawdziwe Win16 ordynaly (KERNEL/USER/GDI)
 *
...
static void init_int3f(unsigned short handler_off)
{
    ...
    /* IDT[0xD]: GP fault handler - wyrzuca error_code i IRET.
     * Bez tego: GP fault -> null -> drugi GP -> double fault -> IDT[8] -> crash. */
    hgpf_off = get_gpf_off();
    p = MK_FP(idt_seg, 0x0D * 8);
    p[0] = (unsigned char)(hgpf_off & 0xFF);
    p[1] = (unsigned char)(hgpf_off >> 8);
    p[2] = (unsigned char)(SEL_CODE16 & 0xFF);
    p[3] = (unsigned char)(SEL_CODE16 >> 8);
    p[4] = 0;
    p[5] = 0x86;   /* P=1, DPL=0, type=6 (16-bit interrupt gate) */
    ...
    kprintf("IDT: phys=0x%05lX int3f=0x%04X int21=0x%04X irq0@0x20=0x%04X\n",
            g_idt_phys, handler_off, h21_off, hirq0_off);
}
...
/* ============================================================
 * KCB (Kernel Control Block) i globalny heap - STEP12
 *
...
int main(int argc, char *argv[])
{
    ...
    serial_init();
    kprintf("=== NE Loader STEP14 - GlobalHeap XMS + RAM detection ===\n");
    kprintf("App: %s\n", app_name);
    ...
    /* 6b/6c: zasoby SKI.EXE (RT_STRING + RT_BITMAP) tylko dla SKI.EXE */

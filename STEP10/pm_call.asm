; pm_call.asm - PM glue (STEP10)
;
; Rozszerzenia vs STEP9c:
;   - 15 okien VESA (huge selectors, jak Win3.1 SVGA driver):
;     SEL_VESA_BASE=0xA0, okna 0..14 co 0x08, kazde 64KB
;     okno i: base = g_lfb_phys + i*65536, limit=65535 (ostatnie: 4095)
;   - SEL_FONT przesuniety z 0x98 na 0x118 (za oknami VESA)
;   - SEL_VESA (0x90) zostaje jako 32-bit alias okna 0 (dla pm32_entry)
;
; Selektory GDT:
...
;   0x98: zarezerwowane (bylo SEL_FONT w STEP9)
;   0xA0..0x110: 15 okien VESA po 64KB (SEL_VESA_BASE=0xA0)
;   0x118: SEL_FONT (BIOS font 8x16, 16-bit, limit=4KB)
;
...
SEL_THUNK       equ 0x88
SEL_VESA        equ 0x90   ; 32-bit alias okna 0, dla pm32_entry (fill LFB)
SEL_VESA_BASE   equ 0xA0   ; baza 15 okien 64KB (huge selectors)
VESA_WIN_COUNT  equ 15     ; liczba okien: ceil(640*480*3 / 65536)
SEL_FONT        equ 0x118  ; BIOS font 8x16 (za oknami VESA)
...
.no_app_data:
    ...
    ; SEL_FONT (0x118): base = g_font_phys, limit 4095
    ...
    ; 15 okien VESA (huge selectors): SEL_VESA_BASE=0xA0, co 8
    ; okno i: base = g_lfb_phys + i*65536
    ;         limit = 65535 (okna 0..13) lub 4095 (okno 14 - ostatnie)
    mov  ebx, [_g_lfb_phys]    ; EBX = baza LFB
    xor  si, si                 ; SI = numer okna (0..14)
.vwin_patch_loop:
    ...
    ; EAX = g_lfb_phys + si * 65536
    xor  eax, eax
    mov  ax, si
    shl  eax, 16               ; EAX = si * 65536
    add  eax, ebx              ; EAX = LFB + si*65536

    ; Patch base w deskryptorze GDT
    mov  [cs:di + 2], ax
    shr  eax, 16
    mov  [cs:di + 4], al
    mov  [cs:di + 7], ah

    ; Patch limit: okna 0..13 -> 0xFFFF, okno 14 -> 0x0FFF
    cmp  si, 14
    je   .vwin_last
    mov  word [cs:di + 0], 0xFFFF
    jmp  .vwin_next
.vwin_last:
    mov  word [cs:di + 0], 0x0FFF   ; 4095 = reszta LFB po 14*64KB
.vwin_next:
    inc  si
    jmp  .vwin_patch_loop
.vwin_patch_done:
...
; =====================================================================
str_title   db "STEP10: Hello, Win16!  huge selectors", 0
str_mode    db "640x480  24bpp  15 x 64KB windows (0xA0..0x110)", 0
str_font    db "SEL_FONT=0x118  SEL_VESA_BASE=0xA0", 0
...
gdt_vesa:                   ; 0x90 SEL_VESA (32-bit data, base=g_lfb_phys, limit=1MB)
    dw 0xFFFF, 0x0000       ; alias okna 0 - uzywany przez pm32_entry (fill 32-bit)
    db 0x00, 10010010b, 01001111b, 0x00

                            ; 0x98 - zarezerwowane (bylo SEL_FONT w STEP9)
    dq 0

; 15 okien VESA (huge selectors), kazde 64KB
; okna 0..13: limit=0xFFFF; okno 14: limit=0x0FFF (reszta LFB)
; Wszystkie base/limit patchowane dynamicznie w patch_gdt
gdt_vwin0:                  ; 0xA0 = SEL_VESA_BASE + 0*8
    dw 0xFFFF, 0x0000
    db 0x00, 10010010b, 00000000b, 0x00
gdt_vwin1:                  ; 0xA8
    dw 0xFFFF, 0x0000
...
gdt_vwin13:                 ; 0x108
    dw 0xFFFF, 0x0000
    db 0x00, 10010010b, 00000000b, 0x00
gdt_vwin14:                 ; 0x110 - ostatnie okno, limit patchowany na 0x0FFF
    dw 0x0FFF, 0x0000
    db 0x00, 10010010b, 00000000b, 0x00

gdt_font:                   ; 0x118 SEL_FONT (16-bit data, base=g_font_phys, limit=4095)
    dw 0x0FFF, 0x0000
    db 0x00, 10010010b, 00000000b, 0x00

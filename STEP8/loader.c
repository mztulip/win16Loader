/*
 * loader.c - STEP8a: NE Loader z KERNEL.EXE + USER.EXE jako DLL
 *
 * Rozszerzenia vs STEP7:
 *  - Laduje USER.EXE jako drugi DLL (dll_idx=1, SEL_DLL_CODE=0x58, DATA=0x60)
 *  - Laduje win16app.exe jako aplikacje testowa
 *
 * Kompilacja:
...
int load_ne(const char *filename)
{
    ...
    /* Informacje o segmentach do 2. przebiegu (fixupy) */
    {
        long             seg_reloc_off[MAX_DLL_SEGS];
        unsigned char __far *seg_data_ptr[MAX_DLL_SEGS];
        unsigned short   seg_file_size[MAX_DLL_SEGS];
        ...
        /* --- Przebieg 1: zaladuj segmenty, przypisz selektory --- */
        for (i = 0; i < ne.ne_cseg && i < MAX_DLL_SEGS; i++) {
            long          file_off;
            ...
            /* Przypisz selektory: kod apki = SEL_APP_CODE, dane = SEL_APP_DATA */
            if (ne.ne_cs != 0 && i == (unsigned)(ne.ne_cs - 1)) {
        ...
        /* --- Przebieg 2: aplikuj fixupy (znamy juz selektory) --- */
        for (i = 0; i < ne.ne_cseg && i < MAX_DLL_SEGS; i++) {
...
{
    serial_init();
    kprintf("=== NE Loader STEP8a - USER.EXE + RegisterClass/CreateWindow ===\n");

    /* 0. Inicjalizacja bufora thunkow i IDT */
    kprintf("--- Init INT 3F ---\n");
    init_int3f(get_int3f_off());

    /* 1. Zaladuj KERNEL.EXE jako DLL (dll_idx=0) */
    ...
    /* 2. Zaladuj USER.EXE jako DLL (dll_idx=1) */

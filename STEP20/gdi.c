...
/* g_skip: pomija pierwsze N wywolan (zeby pominac WM_CREATE atlas building) */
                            ...
                            HDC hdcSrc, int xSrc, int ySrc, unsigned long dwRop)
{
    ...
    /* Trace: log BitBlt call (pomijamy pierwsze g_skip wywolan) */
    if (g_skip > 0) { g_skip--; }
    else if (g_tc < TC_MAX) {
            ...
            for (row1 = 0; row1 < h; row1++) {
                ...
                for (col1 = 0; col1 < w; col1++) {
                    ...
                    if (!(pix1 & DECODE_TRANSP)) {
                        /* Transparentne piksele zostaja alpha=0 — Case C je
                         * pomija, wiec tlo (inne sprity) jest widoczne przez
                         * przezroczyste obszary gondoli. Gondola na wierzchu
                         * z prawdziwą przezroczystoscia (jak w Win3.1). */
    ...
    /* ---- Przypadek 1b: sprite (1..86) -> memDC, NOTSRCCOPY: maska odwrotna ----
     * Laduje odwrocona maske do DC3/DC5: idx15(bialy)=>czarny(0,0,0),
     * inne kolory=>NOT(kolor). Wszystkie piksele alpha=1 — maska jest opaque.
     * Potrzebne do SRCPAINT(DC3->DC6): czarny OR dst=dst, bialy OR dst=0xFF. */
    if (src_hbm >= 1 && src_hbm <= MAX_BITMAPS &&
        hdcDst >= 2 && hdcDst < 16 &&
        dwRop == 0x00330008UL) {
        unsigned buf_sel1b = g_dc_buf_sel[hdcDst];
        if (buf_sel1b != 0) {
            int row1b, col1b;
            int dc_w1b = g_dc_buf_w[hdcDst];
            int dc_h1b = g_dc_buf_h[hdcDst];
            unsigned long dc_pitch1b = (unsigned long)dc_w1b * 4UL;
            for (row1b = 0; row1b < h; row1b++) {
                int by1b = yDst + row1b;
                if (by1b < 0 || by1b >= dc_h1b) continue;
                for (col1b = 0; col1b < w; col1b++) {
                    int bx1b = xDst + col1b;
                    unsigned long pix1b, bo1b;
                    unsigned char r1b, g1b, b1b;
                    if (bx1b < 0 || bx1b >= dc_w1b) continue;
                    pix1b = decode_4bpp_pixel(src_hbm, xSrc+col1b, ySrc+row1b);
                    bo1b = (unsigned long)by1b * dc_pitch1b
                         + (unsigned long)bx1b * 4u;
                    if (pix1b & DECODE_TRANSP) {
                        /* idx15=bialy => NOT(0xFF,0xFF,0xFF) = czarny (0,0,0) */
                        r1b = 0x00; g1b = 0x00; b1b = 0x00;
                    } else {
                        r1b = ~(unsigned char)(pix1b >> 16);
                        g1b = ~(unsigned char)(pix1b >> 8);
                        b1b = ~(unsigned char)pix1b;
                    }
                    draw_dc_pixel(buf_sel1b, bo1b, r1b, g1b, b1b);
                    /* draw_dc_pixel zawsze ustawia alpha=1 */
                }
            }
            g_dc_has_bg[hdcDst] = 1;
        }
            ...
            for (row2 = 0; row2 < h; row2++) {
                ...
                for (col2 = 0; col2 < w; col2++) {
                            ...
                            (unsigned)(src_off & 0xFFFFUL));
                    {
                        ...
                        /* przezroczysty src: nie zmieniaj dst */
                    }
                }
            }
            g_dc_has_bg[hdcDst] = 1;
    ...
    if (hdcDst >= 2 && hdcDst < 16 &&
        ...
        if (dst_buf != 0) {
            ...
            } else if (hdcSrc >= 2 && hdcSrc < 16) {
                /* Zrodlo: memDC z buforem pikselowym — SRCPAINT lub SRCAND. */
                unsigned src_buf = g_dc_buf_sel[hdcSrc];
                if (src_buf != 0) {
                    int row, col;
                    ...
                    for (row = 0; row < h; row++) {
                        ...
                        for (col = 0; col < w; col++) {
                            ...
                            unsigned char __far *sp, __far *dp;
                            ...
                            sp = (unsigned char __far *)MK_FP(
                                    src_buf + (unsigned)(src_off >> 16) * 8u,
                                    (unsigned)(src_off & 0xFFFFUL));
                            if (!sp[3]) continue;  /* przezroczysty src: pomijaj */
                            dst_off = (unsigned long)dy * dst_pitch
                                    + (unsigned long)dx * 4u;
                            dp = (unsigned char __far *)MK_FP(
                                    dst_buf + (unsigned)(dst_off >> 16) * 8u,
                                    (unsigned)(dst_off & 0xFFFFUL));
                            if (dwRop == 0x00EE0086UL) {  /* SRCPAINT: bitwise OR */
                                dp[0] = dp[0] | sp[0];
                                dp[1] = dp[1] | sp[1];
                                dp[2] = dp[2] | sp[2];
                                /* alpha=1 tylko gdy src wnosi kolor (sp!=czarny).
                                 * Czarny src (maska=0) nie zmienia alpha dst —
                                 * zachowuje przezroczystosc tla w obszarach poza
                                 * gondola (gdzie maska DC3 jest czarna). */
                                if (sp[0] | sp[1] | sp[2]) dp[3] = 1;
                            } else {  /* SRCAND z memDC: kopiuj kolor (nie AND).
                                 * Gdy SRCPAINT(maska) dzialal: dp=0xFF, 0xFF AND kolor = kolor.
                                 * Gdy SRCPAINT byl no-op (DC3 puste): COPY daje kolor bezposrednio.
                                 * Efekt: gondola NA WIERZCHU tytulu, transparent pomijany (sp[3]=0
                                 * juz obsluzone przez !sp[3] continue powyzej). */
                                dp[0] = sp[0];
                                dp[1] = sp[1];
                                dp[2] = sp[2];
                                dp[3] = 1;

...
#define GDYN_MAX_SEL   ((unsigned short)0x528)  /* 0x130 + 127*8 */
 ...
 * ============================================================ */
unsigned __far __pascal GetCurrentTask(void)               { return 0; }
...
/* ============================================================
 * ordinal 49: GetModuleFileName
 * Watcom clibs startup wywoluje to by odczytac sciezke do EXE.
 * Stub: zwraca pusty string i dlugosc 0.
 * ============================================================ */
/* Watcom clibs wywoluje: push ds (hMod), push near_off (buf), push nSize.
 * Sygnatura 3 slow (nie 4) -- nSize=0x90=144, nearBuf=0xb3 w DGROUP apki. */
unsigned __far __pascal GetModuleFileName(unsigned hMod,
                                           unsigned nearBuf, unsigned nSize)
{
    unsigned char __far *p;
    (void)nSize;
    if (nearBuf == 0) return 0;
    p = (unsigned char __far *)MK_FP(hMod, nearBuf);
    p[0] = '\0';
    return 0;
}

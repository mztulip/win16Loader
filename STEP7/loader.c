/*
 * loader.c - STEP6: NE Loader z obsluga segmentow danych DLL
 *
 * Rozszerzenia vs STEP5:
 *  - load_ne_dll: laduje WSZYSTKIE segmenty DLL (kod + dane)
 *  - apply_fixups: obsluguje INTERNALREF (SEG16) i IMPORTORDINAL (FAR_PTR)
 *  - INTERNALREF: patchuje MOV AX, DGROUP_placeholder z prawdziwym selektorem
 *  - Dynamiczne przydzielanie selektorow GDT (SEL_DLL_CODE/DATA(i))
 *  - Globalne tablice dla pm_call.asm: g_dll_code_phys[], g_dll_data_phys[] etc.
 *
 * Kompilacja:
 ...
 *
 * seg_sels[i]  = selektor GDT dla segmentu i+1 (0 = brak selektora)
 * num_segs     = liczba segmentow DLL
 * ============================================================ */
                          ...
                          unsigned short num_segs)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (rtype == RTYPE_INTERNALREF) {
            /* INTERNALREF: patch selektora segmentu wewnatrz DLL */
            ...
            /* IMPORTORDINAL: patch far ptr do thunku INT 3F dla funkcji DLL */
...
/* ============================================================
 * load_ne_dll: laduje NE DLL (wszystkie segmenty) i rejestruje eksporty
 * ============================================================ */
int load_ne_dll(const char *filename, const char *modname)
{
    ...
    /* Informacje o segmentach do drugiego przejscia (fixupy) */
    ...
    unsigned short   seg_sels[MAX_DLL_SEGS];  /* selektor per segment (1-based idx) */
    ...
    for (i = 0; i < ne.ne_cseg && i < MAX_DLL_SEGS; i++) {
        ...
        /* Zainicjuj wpisy dla tego segmentu */
        seg_reloc_off[i] = 0;
        seg_data_ptr[i]  = 0;
        seg_file_size[i] = 0;
        seg_has_reloc[i] = 0;
        ...
        /* Zapisz info o segmencie do drugiego przejscia */
        seg_data_ptr[i]  = dst;
        seg_file_size[i] = file_size;
        seg_reloc_off[i] = file_off + file_size;   /* reloc table: tuz po danych */
    ...
    /* ---- Przejscie 2: aplikuj fixupy (gdy znamy juz wszystkie selektory) ---- */
    for (i = 0; i < ne.ne_cseg && i < MAX_DLL_SEGS; i++) {
...
/* ============================================================
 * load_ne: laduje aplikacje NE (wszystkie segmenty + fixupy importow)
 * ============================================================ */
int load_ne(const char *filename)
{
    ...
    for (i = 0; i < ne.ne_cseg; i++) {
        ...
        /* Aplikuj fixupy importow (tylko IMPORTORDINAL; apka nie ma INTERNALREF) */
        if ((seg.ns_flags & NSRELOC) && file_size > 0) {
            unsigned short dummy_sels[1] = { 0 };
            apply_fixups(f, file_off + file_size, dst, (unsigned short)file_size,
                         dummy_sels, 0);
        }
...
{
    serial_init();
    kprintf("=== NE Loader STEP7 - INT 3F DLL thunk ===\n");

    /* 0. Inicjalizacja bufora thunkow i IDT */
    kprintf("--- Init INT 3F ---\n");
    init_int3f(get_int3f_off());

    /* 1. Zaladuj KERNEL.EXE jako DLL (z DGROUP) */
    kprintf("--- Ladowanie KERNEL.EXE ---\n");
    if (load_ne_dll("KERNEL.EXE", "KERNEL") != 0)
        return 1;
    kprintf("KERNEL zaladowany: code=0x%05lX data=0x%05lX has_data=%u\n",
            g_dll[0].code_phys, g_dll[0].data_phys, g_dll[0].has_data);

    /* 2. Zaladuj aplikacje NE */
